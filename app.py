...
def predict_prices(car_detalis):
    # Load the saved pipeline
    car_details['manufacturer']=car_details['name'].apply(lambda x:x.split(' ')[0])
    ...
    # Make predictions
    predictions = pipeline.predict(car_details)

    car_details['predicted_selling_price'] = predictions
    return car_details['predicted_selling_price']
...
def main():
    ...
    car_details = pd.DataFrame(data,index=[0])
